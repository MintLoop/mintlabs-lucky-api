...
function renderGameOptions(selectEl, list) {
    selectEl.innerHTML = list
        .map((g) => `<option value="${g.code}">${g.name}</option>`)
        .join('');
}
...
export function initLucky() {
    ...
    const ensureGames = async () => {
        ...
        if (games.length) {
            ...
            if (_gameSel.value)
                await loadFacts(_gameSel.value);
        }
        return games;
    };
    (async () => {
        ...
        _form.addEventListener('submit', async (e) => {
            ...
            if (mode === 'sum_target') {
                ...
                if (targetSum)
                    payload.target_sum = parseInt(String(targetSum), 10);
            }
            else if (mode === 'birthday') {
                const birthDate = fd.get('birth_date');
                if (birthDate)
                    payload.birth_date = String(birthDate);
            }
            else if (mode === 'lucky') {
                ...
                if (luckyNumbers) {
                    ...
                    if (numbers.length > 0)
                        payload.lucky_numbers = numbers;
                }
            }
            else if (mode === 'wheel') {
                const wheelType = fd.get('wheel_type');
                if (wheelType)
                    payload.wheel_type = String(wheelType);
            }
            const requests = Array.from({ length: sets }, () => fetch(GENERATE_ENDPOINT, {
                method: 'POST',
            ...
            results.forEach((result, idx) => {
                ...
                const numbersHtml = Array.isArray(d.numbers)
                    ? d.numbers.map((n) => `<span class="text-emerald-400 font-semibold">${n}</span>`).join(', ')
                    : (d.numbers || '—');
                ...
                if (d.last_number_info) {
                    let infoText = '';
                    try {
                        if (typeof d.last_number_info === 'string')
                            infoText = d.last_number_info;
                        else if (Array.isArray(d.last_number_info))
                            infoText = d.last_number_info.join(', ');
                        else if (d.last_number_info.message)
                            infoText = String(d.last_number_info.message);
                        else if (d.last_number_info.latency_ms)
                            infoText = `Latency: ${d.last_number_info.latency_ms}ms`;
                        else
                            infoText = JSON.stringify(d.last_number_info);
                    }
                    catch {
                ...
                const singleOddsDisplay = d.odds ||
                    (d.combined_odds
