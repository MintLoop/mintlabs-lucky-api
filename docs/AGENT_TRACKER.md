...
| Demo tests — skip for CI | **GitHub Copilot** | `chore/skip-demo-tests-ci` | 🔁 PR open | Skipped `tests/demo/*` that depend on `/demo/home-v2-polish` (page removed/ignored). Tests can be restored when demo page returns; components remain in production. PR: https://github.com/MintLoop/mintlabs-lucky-api/pull/new/chore/skip-demo-tests-ci |
| Security: upgrade esbuild (GHSA-67mh-4wv8-2f99) | **GitHub Copilot** | `chore/upgrade-esbuild` | 🔁 PR open | Upgraded `esbuild` to `^0.27.2` and updated `package-lock.json` to address GHSA-67mh-4wv8-2f99; `npm audit` shows no remaining vulnerabilities locally. PR: https://github.com/MintLoop/mintlabs-lucky-api/pull/new/chore/upgrade-esbuild |
