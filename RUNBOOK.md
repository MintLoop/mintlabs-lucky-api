...
Tip: If preview builds fail with a missing API base URL, ensure `PUBLIC_API_BASE` (for browser) and `BACKEND_ORIGIN` (for build/SSR) are set directly in the **Project → Settings → Environment Variables** for the frontend project. Avoid legacy team-level `@secret` references.
